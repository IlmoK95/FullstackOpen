...
const Blog = ( props ) => {
  ...
  const removeVisible = { display : props.currentUser === props.blog.user[0].username? '' : 'none'}
  ...
  const handleLike =()=>{
    ...
    props.handleNewLike({'user' : props.blog.user[0].id,
                          'likes' : newLikes,
  ...
  return (
    <div>
      ...
      <div id="blogVisible" style={blogsVisible}>
        {props.blog.title} {props.blog.author}<button onClick={blogVisibility}>hide</button>
        <br></br>
        {props.blog.url} 
        <br></br>
